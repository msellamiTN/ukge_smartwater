...
class TFParts(object):
    ...
    def build_basics(self):
        ...
        with tf.compat.v1.variable_scope("graph", initializer=tf.compat.v1.truncated_normal_initializer(0, 0.3)):
            # Variables (matrix of embeddings/transformations)
            self._ht = ht = tf.compat.v1.get_variable(
                name='ht',  # for t AND h
                shape=[self.num_cons, self.dim],
                dtype=tf.float32)

            self._r = r = tf.compat.v1.get_variable(
                name='r',
            ...
            # for uncertain graph
            self._A_w = tf.compat.v1.placeholder(
                dtype=tf.float32,
                shape=[self.batch_size],
                name='_A_w')

            self._A_neg_hn_index = A_neg_hn_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=(self.batch_size, self._neg_per_positive),
                name='A_neg_hn_index')
            self._A_neg_rel_hn_index = A_neg_rel_hn_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=(self.batch_size, self._neg_per_positive),
                name='A_neg_rel_hn_index')
            self._A_neg_t_index = A_neg_t_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=(self.batch_size, self._neg_per_positive),
                name='A_neg_t_index')
            self._A_neg_h_index = A_neg_h_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=(self.batch_size, self._neg_per_positive),
                name='A_neg_h_index')
            self._A_neg_rel_tn_index = A_neg_rel_tn_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=(self.batch_size, self._neg_per_positive),
                name='A_neg_rel_tn_index')
            self._A_neg_tn_index = A_neg_tn_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=(self.batch_size, self._neg_per_positive),
                name='A_neg_tn_index')
            ...
            # psl batches
            self._soft_h_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._soft_size],
                name='soft_h_index')
            self._soft_r_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._soft_size],
                name='soft_r_index')
            self._soft_t_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._soft_size],
                name='soft_t_index')

            # for uncertain graph and psl
            self._soft_w =  tf.compat.v1.placeholder(
                dtype=tf.float32,
                shape=[self._soft_size],
    ...
    def build_optimizer(self):
        ...
        self._opt = opt = tf.compat.v1.train.AdamOptimizer(lr)
        ...
        # Saver
        self._saver = tf.compat.v1.train.Saver(max_to_keep=2)
